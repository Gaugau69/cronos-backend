"""
app/routers/fitbit.py — Routes OAuth Fitbit Web API
"""
...
from app.services.fitbit_auth import exchange_code_for_token, get_fitbit_auth_url, save_fitbit_token

log = logging.getLogger(__name__)
router = APIRouter(prefix="/auth/fitbit", tags=["fitbit"])


@router.get("/login")
async def fitbit_login(email: str, peakflow_email: str = ""):
    state_data = json.dumps({"email": email, "peakflow_email": peakflow_email or email})
    state = base64.urlsafe_b64encode(state_data.encode()).decode()
    return RedirectResponse(url=get_fitbit_auth_url(state=state))
...
async def fitbit_callback(
    ...
    ok = await save_fitbit_token(db, peakflow_email, email, token_data)
    if not ok:
        return HTMLResponse(_error_page("Erreur lors de la sauvegarde."))

    log.info(f"✓ Fitbit connecté (peakflow: {peakflow_email})")
...
@router.get("/status")
async def fitbit_status(name: str, db: AsyncSession = Depends(get_db)):
    user = (await db.execute(select(User).where(User.email == name))).scalar_one_or_none()
    if not user or not user.token_json:
        return JSONResponse({"connected": False})
    try:
        return JSONResponse({"connected": json.loads(user.token_json).get("provider") == "fitbit"})
    except Exception:
        return JSONResponse({"connected": False})
...
def _success_page(name: str) -> str:
    ...
    <body><div class="card"><div class="check">✓</div><h1>Fitbit connecté !</h1>
