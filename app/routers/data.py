...
SESSIONS_V2 = [
    ...
    {"id":45, "name":"Intervalles 1min", "category":"intensite", "intensity":0.82, "duration_min":45, "distance_km":9, "recovery_cost":0.6, "min_level":1, "description":"1min vite / 1min trot — VO2max accessible, excellent rapport effort/récup.", "example":"45min : 10min échauff → 15×(1min rapide + 1min trot) → 5min retour"},
    {"id":46, "name":"Intervalles 2min", "category":"intensite", "intensity":0.85, "duration_min":50, "distance_km":10, "recovery_cost":0.65, "min_level":1, "description":"2min à allure vive / 2min trot — développe endurance anaérobie.", "example":"50min : 10min échauff → 8×(2min effort + 2min trot) → 10min retour"},
    {"id":47, "name":"Intervalles 3min", "category":"intensite", "intensity":0.88, "duration_min":55, "distance_km":11, "recovery_cost":0.7, "min_level":1, "description":"3min au seuil anaérobie / 3min trot — séance clé VO2max.", "example":"55min : 12min échauff → 6×(3min effort + 3min trot) → 10min retour"},
    {"id":48, "name":"Intervalles 5min", "category":"intensite", "intensity":0.78, "duration_min":60, "distance_km":12, "recovery_cost":0.7, "min_level":2, "description":"5min à allure soutenue / 3min trot — endurance à haute intensité.", "example":"60min : 12min échauff → 5×(5min effort + 3min trot) → 8min retour"},
    {"id":49, "name":"Répétitions 800m", "category":"intensite", "intensity":0.87, "duration_min":55, "distance_km":10, "recovery_cost":0.65, "min_level":1, "description":"800m à allure 3-5km avec récup marchée — précision de l'allure.", "example":"55min : 15min échauff → 6-8×800m (récup 400m marche) → 10min retour"},
    {"id":50, "name":"Répétitions 1500m", "category":"intensite", "intensity":0.82, "duration_min":60, "distance_km":12, "recovery_cost":0.65, "min_level":2, "description":"1500m à allure 5-10km — renforce l'endurance à allure rapide.", "example":"60min : 15min échauff → 4-5×1500m (récup 3min trot) → 10min retour"},

    # ── RÉCUPÉRATION ACTIVE VARIÉE ──────────────────────────────────────────
    {"id":51, "name":"Aqua-jogging", "category":"recuperation", "intensity":0.3, "duration_min":45, "distance_km":0, "recovery_cost":0.0, "min_level":0, "description":"Course dans l'eau — impact zéro, parfait après blessure ou grosse semaine.", "example":"45min en piscine avec ceinture de flottaison"},
    {"id":52, "name":"Vélo tempo", "category":"intensite", "intensity":0.6, "duration_min":60, "distance_km":0, "recovery_cost":0.45, "min_level":1, "description":"Vélo à allure tempo — maintient la condition sans l'impact running.", "example":"60min : 10min échauff → 35min à allure tempo vélo → 15min retour"},
    {"id":53, "name":"Elliptique intervalle", "category":"intensite", "intensity":0.65, "duration_min":45, "distance_km":0, "recovery_cost":0.4, "min_level":1, "description":"Intervalles sur elliptique — cardio sans impact, idéal en récup active.", "example":"45min : 8min échauff → 6×(3min effort + 2min facile) → 5min retour"},
    {"id":54, "name":"Marche nordique", "category":"endurance", "intensity":0.3, "duration_min":60, "distance_km":8, "recovery_cost":0.1, "min_level":0, "description":"Marche avec bâtons — travail global du corps, récupération active.", "example":"60min en terrain varié avec bâtons"},

    # ── SPÉCIFIQUE ENVIRONNEMENT ────────────────────────────────────────────
    {"id":55, "name":"Sortie chaleur", "category":"endurance", "intensity":0.35, "duration_min":40, "distance_km":6, "recovery_cost":0.4, "min_level":0, "description":"Entraînement par forte chaleur — allure réduite, hydratation renforcée, adaptation thermique.", "example":"40min tôt matin ou tard soir, eau toutes les 15min, allure -20%"},
    {"id":56, "name":"Sortie froid", "category":"endurance", "intensity":0.4, "duration_min":45, "distance_km":7, "recovery_cost":0.25, "min_level":0, "description":"Course hivernale — technique de course adaptée au froid, échauffement prolongé.", "example":"45min : 15min échauff progressif → 25min allure modérée → 5min retour calme"},
    {"id":57, "name":"Entraînement altitude", "category":"endurance", "intensity":0.45, "duration_min":60, "distance_km":8, "recovery_cost":0.5, "min_level":1, "description":"Sortie en altitude — effort perçu élevé, fréquence cardiaque haute, allure réduite.", "example":"60min en montagne, allure Z2, s'arrêter si FC > 85% FC max"},

    # ── TRAVAIL TECHNIQUE SPÉCIFIQUE ────────────────────────────────────────
    {"id":58, "name":"Côtes sprint", "category":"force", "intensity":0.9, "duration_min":40, "distance_km":5, "recovery_cost":0.6, "min_level":0, "description":"Sprints en côte très courts (8-12s) — puissance explosive, économie de foulée.", "example":"40min : 15min échauff → 10×(10s sprint montée + descente marchée) → 15min retour"},
    {"id":59, "name":"Descente technique", "category":"specifique", "intensity":0.5, "duration_min":60, "distance_km":10, "recovery_cost":0.45, "min_level":1, "description":"Travail spécifique en descente — fréquence de foulée, freinage actif, technique trail.", "example":"60min trail avec focus sur les descentes : foulées courtes, regard 5m devant"},
    {"id":60, "name":"Dénivelé cumulé", "category":"force", "intensity":0.65, "duration_min":75, "distance_km":10, "recovery_cost":0.6, "min_level":1, "description":"Accumulation de dénivelé positif — répétitions de montées, résistance musculaire.", "example":"75min : 15×montées de 200-300m D+ avec descente trot"},
    {"id":61, "name":"Proprioception", "category":"recuperation", "intensity":0.2, "duration_min":30, "distance_km":0, "recovery_cost":0.1, "min_level":0, "description":"Équilibre, stabilité, pieds nus — prévient les blessures et améliore la foulée.", "example":"30min : équilibre unipodal, réceptions, foulée pieds nus sur herbe"},
    {"id":62, "name":"Gainage core avancé", "category":"force", "intensity":0.4, "duration_min":40, "distance_km":0, "recovery_cost":0.2, "min_level":0, "description":"Gainage profond, transverse, lombaires — stabilité du bassin pour coureurs.", "example":"40min : planche latérale, dead bug, bird dog, anti-rotation"},

    # ── BLOCS DE PÉRIODISATION ──────────────────────────────────────────────
    {"id":63, "name":"Semaine de charge", "category":"endurance", "intensity":0.55, "duration_min":90, "distance_km":18, "recovery_cost":0.7, "min_level":1, "description":"Volume élevé (+20% normal) — phase d'accumulation, fatigue progressive.", "example":"90min Z2, compenser avec plus de sommeil et alimentation adaptée"},
    {"id":64, "name":"Semaine de récupération", "category":"recuperation", "intensity":0.25, "duration_min":35, "distance_km":5, "recovery_cost":0.05, "min_level":0, "description":"Volume réduit (-40%) — relâchement, récupération profonde entre blocs de charge.", "example":"35min très facile, pas de séance intense cette semaine"},
    {"id":65, "name":"Micro-affûtage", "category":"specifique", "intensity":0.45, "duration_min":25, "distance_km":4, "recovery_cost":0.1, "min_level":0, "description":"J-3 à J-1 avant course — activer sans fatiguer, réveiller les jambes.", "example":"25min très facile + 4-6×20s accélérations légères → jambes fraîches"},
    {"id":66, "name":"Grand affûtage", "category":"specifique", "intensity":0.35, "duration_min":30, "distance_km":5, "recovery_cost":0.05, "min_level":1, "description":"J-14 à J-7 : réduction progressive du volume, maintien intensité.", "example":"30min facile avec 2×5min à allure cible — garder les sensations"},
    {"id":67, "name":"Sortie matinale légère", "category":"endurance", "intensity":0.35, "duration_min":30, "distance_km":5, "recovery_cost":0.15, "min_level":0, "description":"Sortie matinale légère — active le métabolisme, améliore la récupération nocturne.", "example":"30min très facile à jeun ou 1h après lever — jamais à fond le matin"},
    {"id":68, "name":"Double séance", "category":"endurance", "intensity":0.45, "duration_min":40, "distance_km":7, "recovery_cost":0.5, "min_level":2, "description":"Deux sorties dans la même journée — matin léger + soir modéré. Réservé aux coureurs expérimentés.", "example":"Matin : 20min facile / Soir : 35min endurance modérée"},
]
